...
class MainWindow(QMainWindow, QAbstractNativeEventFilter):
    ...
    def create_crosshair_tab(self):
        ...
        self.glow_size_slider.setMinimum(1)
        self.glow_size_slider.setMaximum(10)
    ...
    def block_network_upload(self):
        if platform.system() != 'Windows':
            ...
            return
        try:
            rule_name = 'DeltaHelper_BlockGameUpload'
            game_udp_ports = '27015-27030'
            
            # 使用 PowerShell 命令创建防火墙规则
            powershell_script = f'''
            # 检查规则是否存在
            $ruleExists = Get-NetFirewallRule -Name "{rule_name}" -ErrorAction SilentlyContinue
            if (-not $ruleExists) {{
                # 获取活动网卡
                ...
                if ($adapter) {{
                    ...
                    New-NetFirewallRule -Name "{rule_name}" `
                        ...
                        -Protocol UDP `
                        -RemotePort {game_udp_ports} `
                        -Action Block `
                        -Enabled True `
                        -Profile Any
                    Write-Output "规则创建成功"
                }} else {{
                    Write-Output "未找到活动网卡"
                }}
            }} else {{
                Write-Output "规则已存在"
            }}
            ...
            if result.returncode == 0:
                print(f'防火墙规则 {rule_name} 创建成功')
                print(result.stdout)
            else:
                print(f'防火墙规则创建失败: {result.stderr}')
    ...
    def unblock_network_upload(self):
        ...
        try:
            ...
            # 使用 PowerShell 命令删除防火墙规则
            powershell_script = f'''
            # 检查规则是否存在
            $ruleExists = Get-NetFirewallRule -Name "{rule_name}" -ErrorAction SilentlyContinue
            if ($ruleExists) {{
                # 删除规则
                Remove-NetFirewallRule -Name "{rule_name}"
                Write-Output "规则删除成功"
            }} else {{
                Write-Output "规则不存在"
            }}
